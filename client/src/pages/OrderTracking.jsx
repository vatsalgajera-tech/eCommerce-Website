...
import { Search, MapPin, LogIn, Clock, ShieldAlert } from 'lucide-react';
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="Track your Shree Vastra order in real-time." />
      </Helmet>
      ...
      <div style={{ minHeight: '80vh', background: 'var(--color-cream)', padding: '60px 20px' }}>
        <div style={{ maxWidth: '720px', margin: '0 auto' }}>
          ...
          <div style={{ textAlign: 'center', marginBottom: '40px' }}>
            ...
            <h1 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-primary)', fontSize: 'clamp(2rem,5vw,2.8rem)', marginBottom: '8px' }}>
              Track Your Order
            </h1>
            <p style={{ color: 'var(--color-text-muted)' }}>
              Enter your Order ID (e.g., SV123456) to get real-time status
            </p>
          </div>
              ...
              {order && (
                <div style={{ background: 'white', borderRadius: '20px', padding: '32px', boxShadow: 'var(--shadow-card)' }}>
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px', marginBottom: '28px', paddingBottom: '20px', borderBottom: '1px solid var(--color-border)' }}>
                    <div>
                      <h2 style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-primary)', fontSize: '1.4rem', marginBottom: '4px' }}>
                        #{order.orderNumber}
                      </h2>
                  ...
                  {order.items?.length > 0 && (
                    <div>
                      <h3 style={{ fontSize: '0.85rem', fontWeight: 700, marginBottom: '12px', color: 'var(--color-text)' }}>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                        {order.items.map((item, i) => (
                          <div key={i} style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '10px', background: 'var(--color-cream)', borderRadius: '10px' }}>
                            <img
                              src={item.image || item.product?.images?.[0]?.url || 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=60'}
                              alt={item.name}
                              style={{ width: '48px', height: '60px', objectFit: 'cover', borderRadius: '8px', flexShrink: 0 }}
                            />
                            <div style={{ flex: 1 }}>
